...
class User < ActiveRecord::Base
  ...
  validates_presence_of :role_id, :if => :not_using_openid?
  ...
  #errors.add(:login, "cannot contain the characters !@#%*()_-+=")
  ...
  attr_accessible :login, :email, :name, :password, :password_confirmation, :identity_url,:role_id,:state
  
  # has_role? simply needs to return true or false whether a user has a role or not.  
  # It may be a good idea to have "admin" roles return true always
  def has_role?(role)
    list ||= self.roles.collect(&:name)
   return true if list.include?(role.to_s) || list.include?('admin')
  end
